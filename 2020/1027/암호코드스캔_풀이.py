    ...
    for i in range(N):
        ...
        while (j>0):
            ...
            if (b[i][j] == '1' and v[i][j] == 0):
                ...
                c = j
                for k in range(7,-1,-1):  # 8개의 코드 읽어내기
                    cnt = [0, 0, 0]  # 1과 0의 칸수 저장
                ...
                #방문처리
                x = i
                while b[x][j] =='1':    # 행방향이 1일동안
                    for y in range(j, c-1, -1): #왼쪽으로 가면서 확인
                        v[x][y] = 1
                    x += 1
                j = c
            else: # 1이 아니거나 이미 방문한 곳이라면
                j -= 1
    return s
